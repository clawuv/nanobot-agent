...
def _make_provider(config: Config):
    ...
    from nanobot.runtime.modeling import make_provider
...
def gateway(
    ...
    from nanobot.runtime.modeling import resolve_active_model_runtime
    ...
    async def on_cron_job(job: CronJob) -> str | None:
        ...
        try:
            executed = await agent.process_direct(
                reminder_note,
        ...
        response, response_meta = executed
        ...
        if job.payload.deliver and job.payload.to and response:
            should_notify = await evaluate_response(
                ...
                provider,
                agent.model,
                result_payload=response_meta.get("subagent_result") if isinstance(response_meta, dict) else None,
            )
            if should_notify:
    ...
    async def on_heartbeat_execute(tasks: str) -> tuple[str, dict[str, Any]]:
    ...
    # Create API server for desktop/web clients
    from nanobot.gateway.api import GatewayAPI
    api_server = GatewayAPI(
        agent=agent,
        config=config,
        session_manager=session_manager,
    )
    console.print(f"[green]✓[/green] Desktop API: ws://localhost:{port}/api/chat")

    async def run():
        try:
            await api_server.start(host=config.gateway.host, port=port)
            await cron.start()
...
def status():
    ...
    if config_path.exists():
        ...
        console.print(f"Model: {config.get_active_model_profile().model}")
